...
SOURCE_MANIFEST = ROOT / "data/manifests/milestone55_bpbd_events_2023.json"
...
FINAL_MANIFEST = ROOT / "data/manifests/milestone55_bpbd_events_2023_final.json"
...
def main() -> int:
    ...
    if acquisition.get("schema") != "ranah-observatory/milestone55-bpbd-events-2023/v2":
        raise RuntimeError("unsupported M55 acquisition manifest")
    if sha256(SOURCE_CSV) != acquisition["outputs"]["observations"]["sha256"]:
        raise RuntimeError("M55 source-native checksum drift")
    ...
    if len(district_rows) != 19 or len(total_rows) != 1:
        raise RuntimeError("M55 source geography footprint drift")
    ...
    for row in district_rows:
        ...
        if source_name not in aliases:
            raise RuntimeError(f"unmapped M55 geography: {source_name}")
        geography_id, canonical_name = aliases[source_name]
        if geography_id in mapped_ids:
            raise RuntimeError(f"duplicate M55 geography id: {geography_id}")
        ...
        if row_sum != int(float(row["Jumlah"])):
            raise RuntimeError(f"M55 row total mismatch: {source_name}")

    if len(mapped_ids) != 19 or len(out_rows) != 190:
        raise RuntimeError("M55 canonical footprint drift")
    ...
    if canonical_total != source_total:
        raise RuntimeError(f"M55 canonical total mismatch: {canonical_total} != {source_total}")
    ...
    if social_total != source_total:
        raise RuntimeError(f"M55 same-producer event total mismatch: {social_total} != {source_total}")
    ...
    final = {
        "schema": "ranah-observatory/milestone55-bpbd-events-2023-final/v1",
        "milestone": 55,
        "depends_on": [54],
        "source_manifest": {
