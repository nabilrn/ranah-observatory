...
def validate() -> dict:
    ...
    total = [r for r in rows if r["Kode Wilayah"] == "None" and r["Jenis Bencana"] == "Total"]
    districts = [r for r in rows if r not in total]
    assert len(total) == 1 and len(districts) == 19
    assert int(total[0]["Jumlah Kejadian"]) == 1175
    assert sum(int(r["Jumlah Kejadian"]) for r in districts) == 1166
    assert all("KABUPATEN " in r["Jenis Bencana"] or "KOTA " in r["Jenis Bencana"] for r in districts)

    assert final["schema"] == "ranah-observatory/milestone58-bpbd-events-2024-final/v2"
    ...
    assert disagreement["same_producer_monthly_event_total"] == 1175
    assert disagreement["district_rows_reconcile_to_source_total"] is False
    assert disagreement["monthly_total_reconciles_to_source_total"] is True
    ...
    return {"milestone":58,"district_count":19,"source_total_events":1175,"mapped_district_events":1166,"unallocated_difference_events":9,"dashboard_district_filter_ready":True,"complete":True}
