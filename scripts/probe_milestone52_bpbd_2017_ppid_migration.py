...
def summarize_response(result: dict[str, Any]) -> dict[str, Any]:
    ...
    summary["exact_title_present"] = TITLE.casefold() in body.decode("utf-8", errors="ignore").casefold()
...
def extract_asset_hints(url: str, body: bytes) -> list[str]:
    text = body.decode("utf-8", errors="replace")
    hints: list[str] = []
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "schema": "ranah-observatory/m52-bpbd-2017-ppid-migration-live-probe/v1",
    ...
    for url in candidates:
        ...
        if summary["is_pdf"] and raw["body"]:
            name = f"candidate-{len(report['responses']):02d}.pdf"
            target = OUTDIR / name
    ...
    for script_url in script_urls[:MAX_SCRIPT_ASSETS]:
        ...
        if raw["status"] != 200 or not raw["body"]:
            continue
        hints = extract_asset_hints(script_url, raw["body"])
        if hints:
