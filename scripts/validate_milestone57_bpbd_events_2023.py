...
ACQ = ROOT / "data/manifests/milestone57_bpbd_events_2023.json"
FINAL = ROOT / "data/manifests/milestone57_bpbd_events_2023_final.json"
...
EXPECTED_HAZARDS = {
    "Abrasi pantai": 8, "Angin kencang": 562, "Banjir": 144,
    "Banjir Bandang": 10, "Erupsi Gunung Api": 44, "Gelombang Pasang": 1,
    "Gempa Bumi": 1, "Kebakaran Hutan & Lahan": 76, "Kekeringan": 19,
    "Longsor": 166,
}
...
def validate() -> dict:
    ...
    assert acq["schema"] == "ranah-observatory/milestone57-bpbd-events-2023/v1"
    assert acq["milestone"] == 57
    assert acq["depends_on"] == [54, 56]
    ...
    assert len(district_rows) == 19 and len(total_rows) == 1
    assert sum(int(float(r["Jumlah"])) for r in district_rows) == 1031
    assert int(float(total_rows[0]["Jumlah"])) == 1031

    assert final["schema"] == "ranah-observatory/milestone57-bpbd-events-2023-final/v1"
    assert final["milestone"] == 57
    result = final["result"]
    ...
    return {
        "milestone": 57, "district_count": 19, "hazard_count": 10,
        "canonical_row_count": 190, "province_total_events": 1031,
        "same_producer_total_reconciles": True, "dashboard_filter_ready": True,
        "complete": True,
    }
...
def main() -> int:
    try:
        ...
        print(f"M57 validation failed: {exc}", file=sys.stderr)
        return 1
