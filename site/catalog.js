...
function catalogCard(row) {
  ...
  const years = coverage.first_year === coverage.last_year ? `${coverage.first_year}` : `${coverage.first_year}–${coverage.last_year}`;
  const source = row.source_priority.join(" · ");
  const types = row.present_claim_types.map(claimBadge).join("");

  return `
    <article class="catalog-card" data-domain="${catalogEscape(row.domain_label)}" data-search="${catalogEscape([row.public_name,row.source_name,row.id,row.domain_label,source].join(" ").toLocaleLowerCase("id-ID"))}">
...
async function initCatalog() {
  ...
  try {
    ...
    if (data.schema !== "ranah-observatory/public-indicator-catalog/v1") throw new Error(`unsupported indicator catalog schema: ${data.schema}`);
    if (!Array.isArray(data.indicators) || data.indicators.length !== 23) throw new Error("public indicator catalog must contain exactly 23 Panel v3 indicators");
    ...
    filters.innerHTML = domains.map((domain, index) => `<button type="button" data-domain="${catalogEscape(domain)}" aria-pressed="${index === 0 ? "true" : "false"}">${catalogEscape(domain)}</button>`).join("");
    filters.addEventListener("click", (event) => {
      const button = event.target.closest("button[data-domain]");
      if (!button) return;
      filters.querySelectorAll("button[data-domain]").forEach((candidate) => candidate.setAttribute("aria-pressed", candidate === button ? "true" : "false"));
      applyCatalogFilters();
    });
